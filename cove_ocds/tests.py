...
def test_context_api_transform_deprecations():
    ...
    context = {
        "deprecated_fields": {
            "a": {
                "paths": ["path_to_a/0/a", "path_to_a/1/a"],
                "explanation": ["1.1", "description_a"],
            },
            "b": {
                "paths": ["path_to_b/0/b", "path_to_b/1/b"],
                "explanation": ["1.1", "description_b"],
            },
        },
        "validation_errors": [],
        "validation_errors_count": 0,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "xlsx",
            {"exclude_file": True},
            ["results.json", "metatab.json", "unflattened.json"],
        ),
    ],
)
def test_cove_ocds_cli(file_type, options, output):
    ...
    file_name = os.path.join(
        "cove_ocds",
        "fixtures",
        "{}.{}".format("tenders_releases_2_releases", file_type),
    )
...
def test_cove_ocds_cli_validation_errors():
    file_name = os.path.join(
        "cove_ocds", "fixtures", "badfile_all_validation_errors.json"
    )
    test_dir = str(uuid.uuid4())
    output_dir = os.path.join("media", test_dir)
    options = {"output_dir": output_dir, "schema_version": "1.1"}
    call_command("ocds_cli", file_name, **options)

    with open(os.path.join(output_dir, "results.json")) as fp, open(
        os.path.join(
...
def test_cove_ocds_cli_schema_version(version_option):
    test_dir = str(uuid.uuid4())
    file_name = os.path.join(
        "cove_ocds", "fixtures", "tenders_releases_2_releases.json"
    )
    output_dir = os.path.join("media", test_dir)
    ...
    if version_option == "100.100.100":
        with pytest.raises(CommandError):
            call_command(
                "ocds_cli",
                file_name,
                schema_version=version_option,
                output_dir=output_dir,
            )

    else:
        call_command(
            "ocds_cli", file_name, schema_version=version_option, output_dir=output_dir
        )
...
def test_cove_ocds_cli_schema_version_override(file_name, version_option):
    ...
    output_dir = os.path.join("media", test_dir)
    call_command(
        "ocds_cli", file_path, schema_version=version_option, output_dir=output_dir
    )
...
def test_cove_ocds_cli_schema_cache():
    ...
    assert (
        results["schema_url"]
        == "https://standard.open-contracting.org/schema/1__0__3/release-package-schema.json"
    )
    assert sorted(results["validation_errors"], key=lambda k: k["description"])[0][
        "description"
    ].startswith("'id' is missing but required within 'buyer'")
    assert (
        sorted(results["extensions"]["extensions"], key=lambda k: k["name"])[2][
            "description"
        ]
        == "For classifying organizations as micro, sme or large."
    )
    assert (
        sorted(results["extensions"]["invalid_extensions"], key=lambda k: k[0])[0][0]
